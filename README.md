...
This repository independently exercises the Shared CI reusable workflow through `workflow_call`. The workflow dispatch selector and expected results are in `fixtures/cases.json`; it invokes the immutable candidate SHA recorded in `.github/workflows/validate-shared-ci.yml`.
